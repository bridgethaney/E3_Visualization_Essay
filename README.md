...
While the visual design of the map can appeal to user experience, the interactivity and functionality of the project are more aligned with the user interface. On the top and lower part of the screen, there are two bars. The top bar spotlights the variables, the name of the institution providing the information, and methods of changing the map view or downloading the data. When hovering over each variable on the bar, it displays the measurements used and the respective symbology. Clicking the variable turns it off from being shown in the map. As for the lower bar, its main purpose is to control the time lapse, including toggling the speed of the time lapse, and change modes of displaying select data or all data. Both bars impact how the data is viewed and what data is being shown. When any feature is changed or, for instance, the browser window is resized, the map responds to the request immediately.
